...
phi = 1+X*0
#(X-0.5)**2 + (Y-0.5)**2 + Z**2
drivers = {1: [0.2, 0.5, 0.1], 2: [-0.2, 0.1, -0.5],4: [-0.6,-0.1,0.1]} # a dictionary with n entries
base_speeds = [1+X**2, 2+X**2, 3+X**2, 4+X**2, 2.5+X**2,2+X**2,3+X**2,4+X**2] # a list of 2^n speed functions
...
for x in range(taps):
	for y in range(taps):
		for z in range(taps):
			...
			if abs((x-0.5*taps)**2+(y-0.5*taps)**2 - 200) > 50:
				inverse_cylinder.append((x,y,z))
			if abs((x - 0.5*taps)**2 - 400) + (z-0.5*taps)**2 + (y-0.5*taps)**2 < 100:
				phi[x][y][z] = 0

for (x,y,z) in inverse_cylinder:
	for i in range(num_branches):
		base_speeds[i][x][y][z] *= i/20


# add white noise to speeds:
sigma = 0.2 # noise loudness
